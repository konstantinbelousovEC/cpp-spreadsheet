...
Ваша таблица должна:
быть эффективной по памяти в случае, если таблица разрежена (минимальная печатная область большая, а непустых ячеек мало);
...
Любой метод из интерфейса SheetInterface должен работать не дольше, чем за O(K), где К — размер минимальной печатной области. Это касается и тех методов, которые только планируются: удаление и вставка столбцов/строк.
...
ANTLR написан на Java, поэтому для его работы понадобится комплект разработки JDK. Также можете использовать OpenJDK.
